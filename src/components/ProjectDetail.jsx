...
import { SiJavascript, SiHtml5, SiCss3, SiReact, SiExpress, SiTailwindcss, SiPython } from "react-icons/si";
  ...
  useEffect(() => {
    ...
    if (selectedProject) {
      const enhancedProject = {
        ...
        Github: selectedProject.Github || 'https://github.com/vijaydasp',
      };
            ...
            <div className="space-y-6 md:space-y-10 animate-slideInLeft">
              ...
              <div className="space-y-4 md:space-y-6">
                ...
                <div className="space-y-2 md:space-y-4">
                  <div>
                    <h4 className="text-base md:text-lg font-semibold text-blue-300 flex items-center gap-2 mb-1">
                      <Cpu className="w-4 h-4 md:w-5 md:h-5 text-indigo-400" /> Hardware
                    </h4>
                    {project.TechStack && project.TechStack.Hardware && project.TechStack.Hardware.length > 0 ? (
                      <div className="flex flex-wrap gap-2 md:gap-3">
                        {project.TechStack.Hardware.map((tech, index) => (
                          <TechBadge key={index} tech={tech.trim()} />
                        ))}
                      </div>
                    ) : (
                      <p className="text-sm md:text-base text-gray-400 opacity-50">No hardware added.</p>
                    )}
                  </div>
                  <div>
                    <h4 className="text-base md:text-lg font-semibold text-blue-300 flex items-center gap-2 mt-3 mb-1">
                      <Code className="w-4 h-4 md:w-5 md:h-5 text-blue-300" /> Software
                    </h4>
                    {project.TechStack && project.TechStack.Software && project.TechStack.Software.length > 0 ? (
                      <div className="flex flex-wrap gap-2 md:gap-3">
                        {project.TechStack.Software.map((tech, index) => (
                          <TechBadge key={index} tech={tech.trim()} />
                        ))}
                      </div>
                    ) : (
                      <p className="text-sm md:text-base text-gray-400 opacity-50">No software added.</p>
                    )}
                  </div>
                </div>
              </div>
